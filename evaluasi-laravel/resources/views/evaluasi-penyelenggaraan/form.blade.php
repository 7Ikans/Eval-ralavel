...
<head>
    ...
    <style>
        ...
        .warning-box { background-color:#fff3cd; border-radius:4px; padding:20px; margin-top:16px; }
        .warning-title { color:#856404; font-size:18px; margin-bottom:12px; font-weight:600; }
        .warning-list { color:#856404; font-size:13px; padding-left:20px; line-height:1.8; margin-bottom:0; }
        .wajib { color:#dc3545; font-weight:bold; margin-left:2px; }
        .section-title { font-size:15px; font-weight:700; color:#28a745; padding:14px 20px 8px; border-bottom:2px solid #e8f5e9; }
        .section-subtitle { font-size:13px; font-weight:600; color:#555; padding:10px 20px 4px; }
        ...
        .jenis-card:hover { border-color:#28a745; background:#f0fff4; }
        .jenis-card.selected { border-color:#28a745; background:#e6f9ed; box-shadow:0 0 0 3px rgba(40,167,69,0.15); }
        .jenis-card .jenis-icon { font-size:40px; margin-bottom:10px; }
        .jenis-card .jenis-label { font-size:15px; font-weight:700; color:#333; }
        .jenis-card .jenis-desc { font-size:12px; color:#777; margin-top:4px; }
        .jenis-card.selected .jenis-label { color:#28a745; }
        .badge-jenis {
        ...
        .slider-labels span { font-size:11px; color:#aaa; }
        ...
        .btn-submit-custom:hover { background-color:#e0a800; }
        ...
        .btn-cancel-custom:hover { background-color:#727b84; }

        .aspek-section { display:none; }
    </style>
</head>
<body>
...
<div class="container mt-4">
    ...
    <div class="card-custom">
        <div class="data-row"><div class="data-label">NIP</div><div class="data-value">{{ $peserta['nip_peserta'] ?? '-' }}</div></div>
        <div class="data-row"><div class="data-label">Nama</div><div class="data-value">{{ $peserta['nama_peserta'] ?? '-' }}</div></div>
        <div class="data-row"><div class="data-label">Jabatan</div><div class="data-value">{{ $peserta['jabatan'] ?? '-' }}</div></div>
        <div class="data-row"><div class="data-label">Instansi</div><div class="data-value">{{ $peserta['instansi'] ?? '-' }}</div></div>
        <div class="data-row"><div class="data-label">Pelatihan</div><div class="data-value">{{ $peserta['nama_pelatihan'] ?? '-' }}</div></div>
    </div>
    ...
    <div id="wrapperForm" style="display:none;">
        ...
        <form method="POST" action="{{ route('evaluasi-penyelenggaraan.store') }}" id="form-eval-ep">
            ...
            {{-- ======================================================== --}}
            {{-- ASPEK 1: LAYANAN REGISTRASI DAN INFORMASI (Klasikal + Virtual) --}}
            {{-- ======================================================== --}}
            ...
            {{-- ======================================================== --}}
            {{-- ASPEK 2: PROSES PEMBELAJARAN (Klasikal + Virtual) --}}
            {{-- ======================================================== --}}
            ...
            {{-- ======================================================== --}}
            {{-- ASPEK 3: SARANA DAN PRASARANA (Klasikal saja) --}}
            {{-- ======================================================== --}}
            ...
            {{-- ======================================================== --}}
            {{-- ASPEK 4: KONSUMSI (Klasikal saja) --}}
            {{-- ======================================================== --}}
...
<script>
    const mapNilai = { 1:'Tidak Baik', 2:'Kurang Baik', 3:'Baik', 4:'Sangat Baik' };
    ...
    function pilihJenis(jenis) {
        ...
        // Tampilkan aspek yang selalu relevan (Layanan + PBM)
        $('.aspek-section').hide();
        $('#aspek-layanan, #aspek-pbm, #section-submit').show();

        if (jenis === 'klasikal') {
            // Klasikal: TAMPILKAN dan AKTIFKAN input Sarpras & Konsumsi
            ...
            // Virtual: SEMBUNYIKAN dan NONAKTIFKAN input Sarpras & Konsumsi
            $('.aspek-klasikal-only').hide();
            $('.aspek-klasikal-only').find('input, textarea').prop('disabled', true); // <--- Kunci utamanya ada di sini!
        ...
        // Init slider yang baru muncul
        initSliders();

        // Scroll ke aspek pertama
        $('html, body').animate({
    ...
    $(document).ready(function() {
        ...
        // Validasi: pastikan jenis kelas sudah dipilih sebelum submit
